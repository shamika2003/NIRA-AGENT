...
public sealed class OllamaClient : IDisposable
{
    ...
    private readonly SegaOllamaApiKeyService _apiKeys;
    private bool _disposed;

    public OllamaClient(
        SegaOllamaApiKeyService apiKeys)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await SendNonStreamingChatJsonAsync(
            json,
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt < 2; attempt++)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                // signal instead of letting Sega fail later with JSON errors.
                ...
                key = await _apiKeys.ResolveProblemAsync(
                    SegaOllamaCredentialProblem.UsageUnavailable,
                    key,
    ...
    private static SegaOllamaCredentialProblem? ClassifyCredentialProblem(
        HttpStatusCode statusCode,
        string responseBody)
    {
        if (statusCode is HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden)
        {
            return SegaOllamaCredentialProblem.InvalidOrRevoked;
        }
        ...
        {
            return SegaOllamaCredentialProblem.UsageUnavailable;
        }
    ...
    private static string BuildSafeDetail(
        ...
        string responseBody)
    {
        if (problem == SegaOllamaCredentialProblem.InvalidOrRevoked)
        {
