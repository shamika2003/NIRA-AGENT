...
public sealed class NIRABranchWorkStore
{
    ...
    public NIRABranchWorkStore(
        NIRABranchStore branchStore)
    {
        _branchStore =
            branchStore
    ...
    public async Task<IReadOnlyList<NIRABranchWorkItem>>
        ReadAllAsync(
            CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText =
            ...
            FROM NIRA_branch_work_items
            ORDER BY created_at_utc ASC;
            """;

        List<NIRABranchWorkItem> result =
            new();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText =
            """
            INSERT INTO NIRA_branch_work_items (
                id,
                branch_id,
                goal_id,
                kind,
    ...
    private NIRABranchWorkItem ReadItem(
        SqliteDataReader reader)
    {
        ...
        NIRAAgent.Capabilities.NIRACapabilityRequest? capabilityRequest =
            string.IsNullOrWhiteSpace(
                capabilityJson)
                ? null
                : JsonSerializer.Deserialize<NIRAAgent.Capabilities.NIRACapabilityRequest>(
                    capabilityJson,
                    _jsonOptions);

        NIRAAgent.Tools.NIRADynamicToolInvocation? dynamicToolInvocation =
            string.IsNullOrWhiteSpace(
                dynamicToolJson)
                ? null
                : JsonSerializer.Deserialize<NIRAAgent.Tools.NIRADynamicToolInvocation>(
                    dynamicToolJson,
                    _jsonOptions);

        return new NIRABranchWorkItem
        {
            Id =
                Guid.Parse(
                    reader.GetString(
            ...
            Kind =
                (NIRABranchWorkKind)reader.GetInt32(
                    3),

            Status =
                (NIRABranchWorkStatus)reader.GetInt32(
                    4),
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            $"""
            CREATE TABLE IF NOT EXISTS NIRA_branch_work_meta (
                key TEXT PRIMARY KEY,
                value TEXT NOT NULL
            );

            INSERT OR IGNORE INTO NIRA_branch_work_meta (key, value)
            VALUES ('schema_version', '{SchemaVersion}');

            CREATE TABLE IF NOT EXISTS NIRA_branch_work_items (
                id TEXT PRIMARY KEY,
            ...
            CREATE INDEX IF NOT EXISTS idx_NIRA_branch_work_branch
                ON NIRA_branch_work_items(branch_id, status, created_at_utc);

            CREATE INDEX IF NOT EXISTS idx_NIRA_branch_work_status
                ON NIRA_branch_work_items(status, created_at_utc);

            CREATE INDEX IF NOT EXISTS idx_NIRA_branch_work_unnotified
                ON NIRA_branch_work_items(result_notified_at_utc, status, finished_at_utc);
            """;
        ...
        versionCommand.CommandText =
            "SELECT value FROM NIRA_branch_work_meta WHERE key = 'schema_version';";
            ...
            version != SchemaVersion)
        {
            throw new InvalidOperationException(
                $"Unsupported NIRA branch-work schema. Expected {SchemaVersion}, found '{value}'.");
        }
    }
