...
// scripted progress message. It only gives NIRA another cognition
// opportunity when authoritative branch-owned work has remained
// Running for a meaningful amount of time.
//
// NIRA then decides whether to start other independent work, wait,
// speak naturally to the user, or do nothing at all.
// =============================================================

public sealed class NIRABranchWorkReconsiderationService
    : BackgroundService
{
    ...
    public NIRABranchWorkReconsiderationService(
        ...
        CancellationToken stoppingToken)
    {
        ...
        foreach (IGrouping<Guid, NIRABranchWorkItem> group in
                 running.GroupBy(
                     item =>
                         item.GoalId))
        {
            NIRABranchWorkItem[] items =
                group
                    .OrderBy(
                        item =>
                            item.StartedAtUtc)
        ...
        CancellationToken stoppingToken)
    {
        try
        {
            ...
            await _background.ProcessInternalAsync(
                NIRAMindEvent.BranchWorkReconsideration(
                    snapshot),
                stoppingToken);
        }
        catch (OperationCanceledException)
