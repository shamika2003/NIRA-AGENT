...
public sealed class NIRADynamicToolValidator
{
    ...
    public NIRADynamicToolValidator(NIRACapabilityRegistry registry)
    {
        _registry = registry ?? throw new ArgumentNullException(nameof(registry));
    }

    public NIRADynamicToolDefinition ValidateDefinition(NIRADynamicToolDefinition definition)
    {
        ArgumentNullException.ThrowIfNull(definition);
        NIRADynamicToolDefinition normalized = definition.Normalize();
        ...
        if (NIRASensitiveMemoryPolicy.ContainsSensitiveSecret(JsonSerializer.Serialize(normalized)))
        ...
        Dictionary<string, NIRADynamicToolParameterDefinition> parameters =
            new(StringComparer.OrdinalIgnoreCase);
        foreach (NIRADynamicToolParameterDefinition parameter in normalized.Parameters)
        {
        ...
        Dictionary<string, NIRADynamicToolStep> steps =
            new(StringComparer.OrdinalIgnoreCase);
        foreach (NIRADynamicToolStep step in normalized.Steps)
        {
        ...
        foreach (NIRADynamicToolStep step in normalized.Steps)
        {
            if (!_registry.TryResolve(step.CapabilityId, out INIRACapabilityHandler? handler) || handler == null)
            ...
            Dictionary<string, NIRACapabilityParameterDescriptor> capabilityParameters =
            ...
            foreach (NIRACapabilityParameterDescriptor capabilityParameter in descriptor.Parameters)
            {
                if (!step.Arguments.TryGetValue(capabilityParameter.Name, out NIRADynamicToolValueBinding? binding))
                {
    ...
    {
        NIRADynamicToolDefinition normalized = ValidateDefinition(definition);
        NIRADynamicToolInvocation clean = invocation.Normalize();

        Dictionary<string, NIRADynamicToolParameterDefinition> definitions =
        ...
        {
            if (!definitions.TryGetValue(property.Name, out NIRADynamicToolParameterDefinition? parameter))
    ...
    public IReadOnlyList<NIRADynamicToolStep> GetExecutionOrder(NIRADynamicToolDefinition definition)
    {
        NIRADynamicToolDefinition normalized = definition.Normalize();
        Dictionary<string, NIRADynamicToolStep> byId =
        ...
        List<NIRADynamicToolStep> result = new();
        while (ready.Count > 0)
        {
            string id = ready[0];
            ready.RemoveAt(0);
    ...
    private static void ValidateBinding(
        ...
        NIRACapabilityParameterDescriptor target,
        IReadOnlyDictionary<string, NIRADynamicToolParameterDefinition> parameters,
        IReadOnlyDictionary<string, NIRADynamicToolStep> steps)
    {
        switch (binding.Source)
        {
            ...
            case NIRADynamicValueSource.Parameter:
                if (binding.ParameterName == null || !parameters.TryGetValue(binding.ParameterName, out NIRADynamicToolParameterDefinition? parameter))
    ...
    private static void ValidateCondition(
        ...
        IReadOnlyDictionary<string, NIRADynamicToolStep> steps)
    {
        NIRADynamicToolCondition condition = step.Condition;
        switch (condition.Kind)
        {
            case NIRADynamicConditionKind.Always:
                return;

            case NIRADynamicConditionKind.ParameterEquals:
                if (condition.ParameterName == null ||
                    !parameters.TryGetValue(condition.ParameterName, out NIRADynamicToolParameterDefinition? parameter) ||
            ...
            case NIRADynamicConditionKind.StepSucceeded:
            case NIRADynamicConditionKind.StepFailed:
