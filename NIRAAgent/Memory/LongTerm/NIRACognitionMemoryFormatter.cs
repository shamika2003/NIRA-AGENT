...
public static class NIRACognitionMemoryFormatter
{
        ...
        IReadOnlyList<NIRAMemoryKnowledgeEntry>? memories)
    {
        ...
        foreach (
            NIRAMemoryKnowledgeEntry entry
            in memories)
        {
        ...
        int characterBudget)
    {
        ...
        AppendWithinBudget(
            builder,
            "Absence from this map never proves NIRA lacks a memory. Search when exact remembered content could matter.\n\n",
            characterBudget);
        ...
        foreach (
            IGrouping<NIRAMemoryKind, NIRAMemoryKnowledgeEntry> group
            in memories
                .GroupBy(
                    entry =>
                        entry.Memory.Kind)
        ...
        IEnumerable<NIRAMemoryKnowledgeEntry> anchors =
            memories
                .OrderByDescending(
                    entry =>
    ...
    private static void AppendMemory(
        StringBuilder builder,
        NIRAMemoryRecord rawMemory,
        NIRAMemoryAssociationProfile? rawAssociation,
        string? label)
    {
        NIRAMemoryRecord memory =
            rawMemory.Normalize();
        ...
        NIRAMemoryAssociationProfile association =
            (rawAssociation ?? new NIRAMemoryAssociationProfile())
                .Normalize();
