...
namespace NIRAAgent.Character;

public static class NIRACharacterContextFormatter
{
        ...
        NIRAInteractionContext interaction)
    {
        ...
        builder.AppendLine(
            $"Event: {FormatEventName(interaction.Event.EventName)}");
        ...
        builder.AppendLine(
            $"NIRA already responded to this structured " +
            $"topic recently: " +
            $"{interaction.NIRAAlreadyRespondedRecently}");
    ...
    private static void AppendHistory(
        StringBuilder builder,
        IReadOnlyList<NIRASocialEvent> history)
    {
        NIRASocialEvent[] recent =
            history
                .TakeLast(
                    MaximumRecentEvents)
                .ToArray();
            ...
            in recent)
        {
            builder.AppendLine(
                ...
                $"{FormatSource(socialEvent.Source)}/" +
                $"{FormatKind(socialEvent.Kind)} " +
    ...
    {
        return source == NIRASocialEventSource.NIRA
            ? "NIRA"
            : source.ToString();
    }
    ...
    {
        return kind == NIRASocialEventKind.NIRAResponse
            ? "NIRAResponse"
            : kind.ToString();
    }
